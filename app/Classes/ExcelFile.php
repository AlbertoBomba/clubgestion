...
class ExcelFile
{
    ...
    var $records;
    var $aWith;
    var $query; // Query builder opcional

    function __construct($model = null, $filters = [], $columns = [], $filename = '', $sort = [], $aWith = [], $records = null)
    {
        ...
        $this->records = $records; // Permitir pasar registros directamente
    }
    ...
    function generate($table = null)
    {
        ...
        $reader = new Html();
        if (!$table) {
            // Si no se pasaron registros directamente, construir query
            if (!$this->records) {
                $query = $this->model::query();
                
                // Aplicar with si existen
                if (!empty($this->aWith)) {
                    $query->with($this->aWith);
                }
                ...
                if (!empty($this->filters)) {
                    foreach ($this->filters as $key => $value) {
                        if (is_array($value)) {
                            // Filtro con operador: ['field' => ['operator', 'value']]
                            $query->where($key, $value[0], $value[1]);
                        } else {
                            // Filtro simple: ['field' => 'value']
