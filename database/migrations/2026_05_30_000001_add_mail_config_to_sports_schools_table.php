...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('sports_schools', function (Blueprint $table) {
            $table->string('mail_host')->nullable()->after('api_enabled');
            $table->unsignedSmallInteger('mail_port')->nullable()->default(587)->after('mail_host');
            $table->string('mail_encryption', 10)->nullable()->default('tls')->after('mail_port');
            $table->string('mail_username')->nullable()->after('mail_encryption');
            $table->text('mail_password')->nullable()->after('mail_username'); // stored encrypted
