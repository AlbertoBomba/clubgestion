<div>
    ...
    <div class="sticky top-16 z-10 bg-white-pure flex items-center justify-between p-6 border-b border-gray-100">
        ...
        <a href="{{ route('sections.create') }}" class="inline-flex items-center px-4 py-2 rounded-xl text-white font-semibold text-sm shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-300 hover:-translate-y-1 bg-blue-600 hover:bg-blue-700">
    ...
    <div class="card-modern bg-white-pure rounded-b-2xl shadow-xl border border-primary/10 overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            <div class="relative">
                ...
                <input wire:model.live="search" type="text" placeholder="Buscar secciones..." 
                ...
                <tbody class="bg-white-pure divide-y divide-silver/30">
                    @forelse($sections as $section)
                        <tr class="hover:bg-primary/5">
                            <td class="px-6 py-4"><div class="text-sm font-semibold">{{ $section->name }}</div></td>
                            <td class="px-6 py-4"><div class="text-sm text-gray-600">{{ $section->description ?? '-' }}</div></td>
                            <td class="px-6 py-4">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $section->active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-12 text-center text-gray-400">No se encontraron secciones</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        @if($sections->hasPages())
            <div class="px-6 py-4 border-t border-silver/30">{{ $sections->links() }}</div>
        @endif
    </div>

    <x-dialog-modal wire:model="confirmingDeletion">
        <x-slot name="title">Eliminar Sección</x-slot>
        <x-slot name="content">¿Estás seguro de que deseas eliminar esta sección?</x-slot>
        <x-slot name="footer">
            <x-secondary-button wire:click="$set('confirmingDeletion', false)">Cancelar</x-secondary-button>
            <x-danger-button class="ml-3" wire:click="deleteSection" wire:loading.attr="disabled" wire:target="deleteSection">
                <span wire:loading.remove wire:target="deleteSection">Eliminar</span>
                <span wire:loading wire:target="deleteSection" class="inline-flex items-center">
