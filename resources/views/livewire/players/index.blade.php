<div>
                                ...
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    @else
                                        <button wire:click="viewPlayer({{ $player->id }})" 
                                            class="inline-flex items-center px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors duration-200 text-xs font-semibold">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <x-dialog-modal wire:model="showPreviewModal" maxWidth="2xl">
        ...
        <x-slot name="content">
            <div class="space-y-4">
                ...
                <!-- Jugadores inactivos excluidos -->
                @if(count($inactivePlayersExcluded) > 0)
                    <div class="bg-orange-50 border-l-4 border-orange-500 p-4 rounded">
                        <div class="flex items-start gap-3">
                            <svg class="w-6 h-6 text-orange-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <div class="flex-1">
                                <h4 class="text-sm font-semibold text-orange-900 mb-2">
                                    ⚠️ Jugadores Excluidos (Inactivos)
                                </h4>
                                <p class="text-xs text-orange-700 mb-3">
                                    Los siguientes jugadores NO se cambiarán de equipo porque están desactivados:
                                </p>
                                <div class="grid grid-cols-1 gap-2">
                                    @foreach($inactivePlayersExcluded as $player)
                                        <div class="flex items-center gap-3 p-2 bg-white rounded border border-orange-200">
                                            @if($player['photo'])
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <div class="text-xs text-gray-600">DNI: {{ $player['dni'] ?? 'N/A' }}</div>
                                            </div>
                                ...
                                <p class="text-xs text-orange-700 mt-3 italic">
                                    💡 Para cambiarlos de equipo, primero debes activarlos.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <x-slot name="footer">
            <div class="flex items-center justify-between w-full gap-3">
                <x-secondary-button wire:click="closePreviewModal">
                    Cancelar
                </x-secondary-button>
    ...
    <!-- Modal de Visualización de Jugador -->
    <x-dialog-modal wire:model="showPlayerViewModal" maxWidth="2xl">
        ...
        <x-slot name="content">
            @if($playerToView)
                ...
                <div class="bg-gradient-to-r from-primary/10 to-night-blue/10 rounded-xl p-6 mb-6 border border-primary/20">
                    <div class="flex items-center gap-6">
                        <!-- Foto -->
                        <div class="flex-shrink-0">
                            @if($playerToView->player_photo)
                                <img src="{{ asset('storage/' . $playerToView->player_photo) }}" 
                                    alt="{{ $playerToView->full_name }}" 
                                    class="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg">
                            @else
                                <div class="w-24 h-24 rounded-full bg-gradient-to-br from-primary to-night-blue flex items-center justify-center border-4 border-white shadow-lg">
                                    <span class="text-white font-bold text-3xl">{{ substr($playerToView->name, 0, 1) }}{{ substr($playerToView->surname, 0, 1) }}</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <!-- Nombre y estado -->
                        <div class="flex-1">
                            <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $playerToView->full_name }}</h3>
                            <div class="flex items-center gap-3 flex-wrap">
                                @if($playerToView->cod_matricula)
                                    <span class="text-sm text-gray-600">
                                        <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                        </svg>
                                        {{ $playerToView->cod_matricula }}
                                    </span>
                                @endif
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->dni ?: '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Fecha Nacimiento</p>
                                <p class="text-sm font-semibold text-gray-900">
                                    @if($playerToView->dbirth)
                                        {{ \Carbon\Carbon::parse($playerToView->dbirth)->format('d/m/Y') }}
                                    @else
                                        -
                                    @endif
                                </p>
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-900">
                                    @if($playerToView->dbirth)
                                        {{ \Carbon\Carbon::parse($playerToView->dbirth)->age }} años
                                    @else
                                        -
                                    @endif
                                </p>
                            ...
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Año Deportivo</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->dbanio ?: '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Posición</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->position ?: '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Dorsal</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->dorsal ?: '-' }}</p>
                            </div>
                        </div>
                    </div>

                    <!-- Tutor Legal -->
                    @if($playerToView->nametutor || $playerToView->surnametutor)
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <h4 class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3 flex items-center">
                                <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900">{{ $playerToView->nametutor }} {{ $playerToView->surnametutor }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">DNI</p>
                                    <p class="text-sm font-semibold text-gray-900">{{ $playerToView->dnitutor ?: '-' }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Teléfono Principal</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->phone1 ?: '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Teléfono Secundario</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $playerToView->phone2 ?: '-' }}</p>
                            </div>
                            <div class="md:col-span-2">
                                <p class="text-xs text-gray-500 mb-1">Email</p>
                                <p class="text-sm font-semibold text-gray-900 break-all">{{ $playerToView->email ?: '-' }}</p>
                            </div>
                        </div>
                    </div>

                    <!-- Dirección -->
                    @if($playerToView->address || $playerToView->town)
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            <h4 class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3 flex items-center">
                                <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="text-sm text-gray-900">
                                @if($playerToView->address)
                                    <p class="mb-1">{{ $playerToView->address }}</p>
                                @endif
                                <p>
                                    {{ $playerToView->zip ? $playerToView->zip . ' - ' : '' }}{{ $playerToView->town ?: '' }}
                                    @if($playerToView->province)
                                        <span class="text-gray-600">({{ $playerToView->province }})</span>
                                    @endif
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    @foreach($playerToView->teams as $team)
                                        <div class="flex items-center justify-between px-3 py-2 bg-blue-50 rounded-lg">
                                            <span class="text-sm font-semibold text-blue-900">{{ $team->team }}</span>
                                            @if($team->category)
                                                <span class="text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded">{{ $team->category->name }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <!-- Temporadas -->
                        @if($playerToView->seasons->count() > 0)
                            <div class="bg-white border border-gray-200 rounded-lg p-4">
                                <h4 class="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3 flex items-center">
                                    <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($playerToView->seasons as $season)
                                        <span class="inline-flex items-center px-3 py-1 bg-primary/10 text-primary rounded-lg text-xs font-semibold">
                                            {{ $season->season }}
                                        </span>
                                    @endforeach
                                </div>
                    ...
                    @if($playerToView->observations)
                        <div class="bg-white border border-gray-200 rounded-lg p-4">
                            ...
                            <p class="text-sm text-gray-700 whitespace-pre-line">{{ $playerToView->observations }}</p>
                        </div>
                    @endif
                </div>
            @endif
        ...
        <x-slot name="footer">
            <button wire:click="closePlayerViewModal" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors font-semibold">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
