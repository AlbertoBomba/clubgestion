<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-sm text-titanium mt-1">Gestiona los torneos de tu escuela</p>
        </div>
    ...
    <div class="bg-white-pure border border-silver rounded-2xl shadow-sm p-4 mb-6">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar torneos..."
    ...
    @if ($tournaments->isEmpty())
        <div class="text-center py-20 bg-white-pure border border-silver rounded-2xl shadow-sm">
            ...
            <h3 class="text-lg font-semibold text-black-deep mb-1">No hay torneos</h3>
            <p class="text-sm text-titanium mb-6">Crea tu primer torneo para comenzar</p>
            <a href="{{ route('tournaments.create') }}" wire:navigate
                ...
                <div class="bg-white-pure border border-silver rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden group">
                        ...
                        <div class="flex items-center gap-3 min-w-0">
                            ...
                            <div class="min-w-0">
                                ...
                                @if ($tournament->location)
                                    <p class="text-xs text-titanium mt-0.5 truncate">{{ $tournament->location }}</p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="mt-auto border-t border-silver px-5 py-3 flex items-center justify-between gap-2 bg-gray-50/50">
                        ...
                        <a href="{{ route('tournaments.edit', $tournament) }}" wire:navigate
                           class="p-1.5 rounded-lg text-titanium hover:text-primary hover:bg-primary/10 transition-colors" title="Editar">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                        </a>
                        <button wire:click="confirmDelete({{ $tournament->id }})"
                                class="p-1.5 rounded-lg text-titanium hover:text-red-500 hover:bg-red-50 transition-colors" title="Eliminar">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if ($confirmingDeletion)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div class="bg-white-pure rounded-2xl shadow-2xl border border-silver w-full max-w-sm p-6" x-data>
                ...
                <p class="text-sm text-titanium text-center mb-6">Esta acción no se puede deshacer. Se eliminarán también sus fases, equipos y partidos.</p>
