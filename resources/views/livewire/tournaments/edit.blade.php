    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <nav class="flex items-center gap-2 text-xs text-titanium mb-1">
                ...
                <a href="{{ route('tournaments.show', $tournament) }}" wire:navigate class="hover:text-primary transition-colors">{{ $tournament->name }}</a>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-black-deep font-medium">Editar</span>
            </nav>
            <h1 class="text-2xl font-bold text-black-deep">Editar Torneo</h1>
        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="3"
                                  class="w-full px-4 py-2.5 text-sm border border-silver rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-white-pure text-black-deep resize-none"></textarea>
                    </div>
                    <div>
            ...
            <div class="bg-white-pure border border-silver rounded-2xl shadow-sm p-6">
                ...
                <div>
                    ...
                    @elseif ($tournament->logo)
                        <img src="{{ Storage::url($tournament->logo) }}" alt="{{ $tournament->name }}" class="w-24 h-24 object-cover rounded-xl border border-silver mb-3 mx-auto"/>
                    @endif
                    <label class="flex flex-col items-center justify-center border-2 border-dashed border-silver rounded-xl p-4 cursor-pointer hover:border-primary/50 transition-colors">
                        ...
                        <span class="text-xs text-titanium">{{ $tournament->logo ? 'Cambiar imagen' : 'Subir imagen (max 2MB)' }}</span>
