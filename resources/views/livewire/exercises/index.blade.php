<div>
    ...
    <div class="sticky top-16 z-10 bg-white-pure flex items-center justify-between p-6 border-b border-gray-100 rounded-t-2xl shadow-xl border border-primary/10 mb-6">
        <h2 class="font-bold text-2xl text-titanium leading-tight">
            {{ __('Gestión de Ejercicios') }}
        </h2>
        <a href="{{ route('exercises.create') }}" class="inline-flex items-center px-4 py-2 rounded-xl text-white font-semibold text-sm shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-300 hover:-translate-y-1 bg-blue-600 hover:bg-blue-700">
    ...
    <div class="card-modern bg-white-pure rounded-2xl shadow-xl border border-primary/10 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-primary/5 to-night-blue/5">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-titanium mb-2">Tipo de Ejercicio</label>
                    <select wire:model.live="exerciseTypeFilter" class="w-full px-4 py-2 border border-silver rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-sm">
        ...
        <div class="p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @foreach($exercises as $exercise)
                        <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200">
                            <!-- Image -->
                            <div class="bg-gradient-to-br from-primary/10 to-night-blue/10 flex items-center justify-center relative overflow-hidden">
                                @if($exercise->images->count() > 0)
                                    <img src="{{ asset('storage/' . $exercise->images->first()->file_path) }}" 
                                         alt="{{ $exercise->title }}" 
                                         class="w-full h-auto">
                                @else
                                    <svg class="w-16 h-16 text-primary/30 my-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Favorite button -->
                                <button wire:click="toggleFavorite({{ $exercise->id }})" 
                                        class="absolute top-2 left-2 p-2 rounded-full transition-all duration-300 hover:scale-110
                                               {{ $exercise->favoritedBy->contains(auth()->id()) ? 'bg-red-500 text-white' : 'bg-white/80 text-gray-400 hover:text-red-500' }}">
                                    <svg class="w-5 h-5" fill="{{ $exercise->favoritedBy->contains(auth()->id()) ? 'currentColor' : 'none' }}" 
                            ...
                            <!-- Content -->
                            <div class="p-4">
                                <h3 class="text-lg font-bold text-titanium mb-2 truncate">{{ $exercise->title }}</h3>
                                
                                @if($exercise->description)
                                    <p class="text-sm text-gray-600 mb-3 line-clamp-2">{{ Str::limit($exercise->description, 80) }}</p>
                                @endif
                                ...
                                <div class="space-y-2 mb-4">
                                    @if($exercise->exerciseType)
                                        <div class="flex items-center text-sm text-gray-600">
                                            ...
                                            <span class="font-semibold">{{ $exercise->exerciseType->name }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($exercise->category)
                                        <div class="flex items-center text-sm text-gray-600">
                                            ...
                                            <span>{{ $exercise->category->category }}</span>
                                        </div>
                                    @endif

                                    @if($exercise->recommended_players)
                                        <div class="flex items-center text-sm text-gray-600">
                                            <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                                            </svg>
                                            <span>{{ $exercise->recommended_players }} jugadores</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($exercise->recommended_time)
                                        <div class="flex items-center text-sm text-gray-600">
                                            ...
                                            <span>{{ $exercise->recommended_time }} minutos</span>
                                        </div>
                                    @endif

                                    @if($exercise->recommended_age_min || $exercise->recommended_age_max)
                                        <div class="flex items-center text-sm text-gray-600">
                                            <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <span>
                                                @if($exercise->recommended_age_min && $exercise->recommended_age_max)
                                                    {{ $exercise->recommended_age_min }} - {{ $exercise->recommended_age_max }} años
                                                @elseif($exercise->recommended_age_min)
                                                    Desde {{ $exercise->recommended_age_min }} años
                                                @else
                                                    Hasta {{ $exercise->recommended_age_max }} años
                                                @endif
                                            </span>
                                        </div>
                                    @endif
                                    ...
                                    @if($exercise->difficulty)
                                        <div class="flex items-center text-sm text-gray-600">
                                            ...
                                            <span><strong>Dificultad:</strong> {{ $exercise->difficulty }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($exercise->intensity)
                                        <div class="flex items-center text-sm text-gray-600">
                                            ...
                                            <span><strong>Intensidad:</strong> {{ $exercise->intensity }}</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-gray-500">
                                        @if($exercise->images->count() > 0)
                                            <div class="flex items-center">
                                                ...
                                                <span>{{ $exercise->images->count() }}</span>
                                            </div>
                                        @endif
                                        @if($exercise->videos->count() > 0)
                                            <div class="flex items-center">
                                                ...
                                                <span>{{ $exercise->videos->count() }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="text-xs text-gray-500 mb-4">
                                    <span>Creado por: {{ $exercise->user->name }}</span>
                                </div>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button wire:click="confirmDelete({{ $exercise->id }})" 
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No hay ejercicios</h3>
                    <p class="mt-1 text-sm text-gray-500">Comienza creando un nuevo ejercicio.</p>
                    <div class="mt-6">
                        <a href="{{ route('exercises.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary hover:bg-night-blue focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            Nuevo Ejercicio
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-bold text-gray-900">Eliminar Ejercicio</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">¿Estás seguro de que deseas eliminar este ejercicio? Esta acción no se puede deshacer y se eliminarán todas las imágenes y videos asociados.</p>
                                </div>
                            </div>
                        </div>
                    </div>
